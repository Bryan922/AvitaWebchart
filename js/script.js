...
function initializeMarketData() {
    ...
    currentPrice = 0.0003; // Augmentation du prix à 0,0003
    marketCap = 230.0; // Augmentation de la capitalisation à 230M
    totalSupply = 100000; // 100M tokens (fixe)
    volume24h = 2.5; // 2.5M de volume (augmentation)
    holders = 24750; // Augmentation du nombre de détenteurs
    transactions = 187500; // Augmentation du nombre de transactions
    
    // Initialiser l'historique des prix avec de légères variations autour de 0,0003
    priceHistory = [];
    for (let i = 0; i < 60; i++) {
...
function generateChartData() {
    const data = [];
    const labels = [];
    const basePrice = 0.0003; // Prix de base fixé à 0,0003
    ...
    for (let i = 0; i < 60; i++) {
        ...
        // Générer une légère variation autour de 0,0003 (±1%)
        ...
        options: {
            ...
            scales: {
                ...
                y: {
                    ...
                    // Définir une plage autour de 0,0003 pour le graphique
                    suggestedMin: 0.000295,
                    suggestedMax: 0.000305
                }
            }
        }
    });
...
function updateMarketData() {
    ...
    // Prix cible fixé à 0,0003
    const targetPrice = 0.0003;
    
    // Sauvegarder l'ancien prix pour calculer le pourcentage de changement
    const oldPrice = currentPrice;
    
    // Générer une variation de prix naturelle autour de 0,0003
    // 1. Tendance de base selon la phase du marché
    let baseTrend;
    switch (marketPhase) {
        case 'accumulation':
            baseTrend = 0.0000005 + (Math.random() * 0.0000005);
            break;
        case 'markup':
            baseTrend = 0.0000008 + (Math.random() * 0.0000007);
            break;
        case 'distribution':
            baseTrend = 0.0000003 + (Math.random() * 0.0000004);
            break;
        case 'markdown':
            baseTrend = 0.0000001 + (Math.random() * 0.0000002);
            break;
        default:
            baseTrend = 0.0000005;
    }
    ...
    // Limiter le prix pour qu'il reste dans une plage raisonnable (0,0003 à 0,00033)
    ...
    // Calculer la capitalisation boursière avec des variations naturelles autour de 230M
    // Avec une légère variation pour plus de naturel
    const baseMarketCap = 230.0; // Valeur de base de la capitalisation
    const marketCapPctChange = Math.abs(pricePctChange) * 1.2; // Amplifier l'effet du prix sur la capitalisation
    const marketCapVariation = (Math.random() * 1.5); // Variation aléatoire positive jusqu'à 1.5M
    const newMarketCap = baseMarketCap * (1 + marketCapPctChange) + marketCapVariation;
    
    // Limiter la capitalisation entre 230M et 250M
    marketCap = Math.max(230, Math.min(250, newMarketCap)).toFixed(1);
    
    // Calculer le volume sur 24h - variations naturelles
    // Le volume augmente quand le prix change beaucoup (volatilité)
    const volumeBase = 2.5; // Volume de base à 2.5M
    const volumeMultiplier = 1 + (Math.abs(pricePctChange) * 30); // Plus de volume quand le prix bouge
    const randomVolumeFactor = 1 + (Math.random() * 0.2); // +0-20% de variation aléatoire
    volume24h = volumeBase * volumeMultiplier * randomVolumeFactor;
    
    // Limiter le volume entre 2.5M et 4M
    volume24h = Math.max(2.5, Math.min(4, volume24h)).toFixed(2);
