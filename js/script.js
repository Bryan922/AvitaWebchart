...
function generateRandomMarketState() {
    ...
    // Générer un prix aléatoire avec une grande variabilité (0.0001 à 0.01)
    const priceRange = Math.random();
    if (priceRange < 0.3) {
        // 30% de chance d'avoir un prix très bas (0.0001 à 0.0005)
        currentPrice = 0.0001 + (Math.random() * 0.0004);
    } else if (priceRange < 0.7) {
        // 40% de chance d'avoir un prix moyen (0.0005 à 0.002)
        currentPrice = 0.0005 + (Math.random() * 0.0015);
    } else if (priceRange < 0.9) {
        // 20% de chance d'avoir un prix élevé (0.002 à 0.005)
        currentPrice = 0.002 + (Math.random() * 0.003);
    } else {
        // 10% de chance d'avoir un prix très élevé (0.005 à 0.01)
        currentPrice = 0.005 + (Math.random() * 0.005);
    }
...
function updateMarketData() {
    ...
    marketCap = (currentPrice * totalSupply / 1000).toFixed(2); // Diviser par 1000 pour convertir en millions
