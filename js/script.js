...
let currentPrice = 0.000015; // Prix fixé à 0.000015
let initialPrice = 0.000015;
let marketCap = 351; // Capitalisation fixée à 351M
let volume24h = 78.0; // Volume fixé à 78M
let performance7d = 8.7; // Performance sur 7 jours
let holders = 49500; // Nombre de détenteurs doublé
let transactions = 457558; // Nombre de transactions
...
function saveMarketState() {
    ...
    const marketState = {
        currentPrice: 0.000015, // Forcer le prix à 0.000015
        marketCap: 351, // Forcer la capitalisation à 351M
        volume24h: 78.0, // Forcer le volume à 78M
        performance7d,
        holders,
        transactions,
        priceHistory,
...
function loadMarketState() {
    try {
        ...
        if (savedState) {
            ...
            holders = state.holders || 49500;
            transactions = state.transactions || 457558;
...
function initializeMarketData() {
    ...
    currentPrice = 0.000015; // Prix ajusté à 0,000015
    marketCap = 351.0; // Capitalisation ajustée à 351M
    totalSupply = 100000; // 100M tokens (fixe)
    volume24h = 78.0; // Volume ajusté à 78M
    holders = 49500; // Nombre de détenteurs doublé
    transactions = 457558; // Nombre de transactions
    
    // Initialiser l'historique des prix avec de légères variations autour de 0,000015
    priceHistory = [];
    for (let i = 0; i < 60; i++) {
...
function generateChartData() {
    const data = [];
    const labels = [];
    const basePrice = 0.000015; // Prix de base fixé à 0,000015
    ...
    for (let i = 0; i < 60; i++) {
        ...
        // Générer une légère variation autour de 0,000015 (±1%)
        ...
        options: {
            ...
            scales: {
                ...
                y: {
                    ...
                    // Définir une plage autour de 0,000015 pour le graphique
                    suggestedMin: 0.000014,
                    suggestedMax: 0.000016
                }
            }
        }
    });
...
function updateMarketData() {
    ...
    // Prix cible fixé à 0,000015
    const targetPrice = 0.000015;
    
    // Sauvegarder l'ancien prix pour calculer le pourcentage de changement
    const oldPrice = currentPrice;
    
    // Générer une variation de prix naturelle autour de 0,000015
    ...
    switch (marketPhase) {
        case 'accumulation':
            ...
            break;
        case 'markup':
            baseTrend = 0.0000002 + (Math.random() * 0.0000001);
            break;
        case 'distribution':
            baseTrend = 0.00000005 + (Math.random() * 0.00000005);
            break;
        case 'markdown':
            baseTrend = 0.00000002 + (Math.random() * 0.00000002);
            break;
        default:
            baseTrend = 0.0000001;
    }
    ...
    // Limiter le prix pour qu'il reste dans une plage raisonnable (0,000014 à 0,000016)
    const minPrice = 0.000014;
    const maxPrice = 0.000016;
    ...
    // Fixer la capitalisation boursière à 351M avec de très légères variations
    // Variation maximale de ±1M pour donner l'impression de mouvement
    const marketCapVariation = (Math.random() * 2.0) - 1.0;
    marketCap = (351.0 + marketCapVariation).toFixed(1);
    
    // Fixer le volume à 78M avec de très légères variations
    // Variation maximale de ±2M pour donner l'impression de mouvement
    const volumeVariation = (Math.random() * 4.0) - 2.0;
    volume24h = (78.0 + volumeVariation).toFixed(1);
...
function updateUIElements(pricePctChange) {
    ...
    // Mettre à jour la capitalisation boursière - fixée à 351M
    ...
    // Mettre à jour le volume sur 24h - fixé à 78M
