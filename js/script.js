...
let currentPrice = 0.000019; // Prix fixé à 0.000019
let initialPrice = 0.000019;
let marketCap = 160; // Capitalisation fixée à 160M
let volume24h = 46.0; // Volume fixé à 46M
...
function saveMarketState() {
    ...
    const marketState = {
        currentPrice: 0.000019, // Forcer le prix à 0.000019
        marketCap: 160, // Forcer la capitalisation à 160M
        volume24h: 46.0, // Forcer le volume à 46M
        performance7d,
        holders,
        transactions,
        priceHistory,
...
function initializeMarketData() {
    ...
    currentPrice = 0.000019; // Prix ajusté à 0,000019
    marketCap = 160.0; // Capitalisation ajustée à 160M
    totalSupply = 100000; // 100M tokens (fixe)
    volume24h = 46.0; // Volume ajusté à 46M
    holders = 24750; // Nombre de détenteurs
    transactions = 187500; // Nombre de transactions
    
    // Initialiser l'historique des prix avec de légères variations autour de 0,000019
    priceHistory = [];
    for (let i = 0; i < 60; i++) {
...
function generateChartData() {
    const data = [];
    const labels = [];
    const basePrice = 0.000019; // Prix de base fixé à 0,000019
    ...
    for (let i = 0; i < 60; i++) {
        ...
        // Générer une légère variation autour de 0,000019 (±1%)
        ...
        options: {
            ...
            scales: {
                ...
                y: {
                    ...
                    // Définir une plage autour de 0,000019 pour le graphique
                    suggestedMin: 0.000018,
                    suggestedMax: 0.000020
                }
            }
        }
    });
...
function updateMarketData() {
    ...
    // Prix cible fixé à 0,000019
    const targetPrice = 0.000019;
    
    // Sauvegarder l'ancien prix pour calculer le pourcentage de changement
    const oldPrice = currentPrice;
    
    // Générer une variation de prix naturelle autour de 0,000019
    ...
    // Limiter le prix pour qu'il reste dans une plage raisonnable (0,000018 à 0,000020)
    const minPrice = 0.000018;
    const maxPrice = 0.000020;
    ...
    // Fixer la capitalisation boursière à 160M avec de très légères variations
    // Variation maximale de ±1M pour donner l'impression de mouvement
    const marketCapVariation = (Math.random() * 2.0) - 1.0;
    marketCap = (160.0 + marketCapVariation).toFixed(1);
    
    // Fixer le volume à 46M avec de très légères variations
    // Variation maximale de ±2M pour donner l'impression de mouvement
    const volumeVariation = (Math.random() * 4.0) - 2.0;
    volume24h = (46.0 + volumeVariation).toFixed(1);
...
function updateUIElements(pricePctChange) {
    ...
    // Mettre à jour la capitalisation boursière - fixée à 160M
    ...
    // Mettre à jour le volume sur 24h - fixé à 46M
